...
docker build infra/build -t rdglp-build-test && \
  docker run -it \
  --runtime=sysbox-runc \
  --volume "$PWD:/home/ubuntu/build/regex-directed-graph-line-parser" \
  rdglp-build-test '{"RUN_TASK": "test", "GIT_BRANCH": "'"$GIT_BRANCH"'", "ID_RSA": "'"$ID_RSA_CONTENTS_BASE64"'", "MAVENCENTRAL_USERNAME": "'"$MAVENCENTRAL_USERNAME_BASE64"'", "MAVENCENTRAL_PASSWORD": "'"$MAVENCENTRAL_PASSWORD_BASE64"'"}'
